document.addEventListener('DOMContentLoaded', function () {
  ...
  class AppData {
    constructor(targetAmount = 0, budget = 0, budgetDay = 0, budgetMonth = 0, expensesMonth = 0, incomeMonth = 0, income = {}, addIncome = [], expenses = {}, addExpenses = [], deposit = false, percentDeposit = 0, moneyDeposit = 0, period = 3) {
    ...
    start() {
      ...
      this.targetAmount = +targetAmount.value;
      this.getExpenses();
      ...
      localStorage.setItem('appDataCalculator', JSON.stringify(appData));
      for (let key in this) {
    ...
    reset() {
      ...
      localStorage.setItem('appDataCalculator', JSON.stringify(''));
      for (let key in this) {
        deleteCookie(key);
      };
      deleteCookie('isLoad');
    ...
    getTargetMonth() {
      return this.targetAmount / this.budgetMonth;
    };
    getStatusIncome() {
      switch (true) {
  ...
  const a = JSON.parse(localStorage.getItem('appDataCalculator'));
  if (a) {
    let flag = true;
    for (let key in a) {
      if (getCookie(key) !== (a[key] + '')) flag = false;    }
    if (getCookie('isLoad')!='true') flag = false;
    if (flag) {
      Object.assign(appData, a);
      appData.showResult();
      document.querySelectorAll('input[type=text]').forEach(function (item) {
        item.disabled = true;
      });
